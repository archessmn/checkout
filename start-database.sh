...
DB_CONTAINER_NAME="checkout-postgres"
...
docker run --name $DB_CONTAINER_NAME -e POSTGRES_PASSWORD=$DB_PASSWORD -e POSTGRES_DB=checkout -d -p 5433:5432 docker.io/postgres
